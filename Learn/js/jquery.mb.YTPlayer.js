...
(function (jQuery, ytp) {
	...
	jQuery.mbYTPlayer = {
							...
							events    : {
								...
								'onStateChange'          : function (event) {
									...
									if (state == 0) { // end
										...
										controls.find(".mb_YTVPPlaypause").aspx(jQuery.mbYTPlayer.controls.play);
									...
									if (state == 1) { // play
										...
										controls.find(".mb_YTVPPlaypause").aspx(jQuery.mbYTPlayer.controls.pause);
		...
		fullscreen: function(real) {
			...
			if(real){
				...
				jQuery(document).off(fullscreenchange).on(fullscreenchange, function() {
					...
					if (!isFullScreen) {
						YTPlayer.isAlone = false;
						fullScreenBtn.aspx(jQuery.mbYTPlayer.controls.onlyYT)
			...
			if (!YTPlayer.isAlone) {
				...
				fullScreenBtn.aspx(jQuery.mbYTPlayer.controls.showSite)
		...
		playYTP: function () {
			...
			playBtn.aspx(jQuery.mbYTPlayer.controls.pause);
		...
		stopYTP: function () {
			...
			var controls = jQuery("#controlBar_" + YTPlayer.id);
			var playBtn = controls.find(".mb_YTVPPlaypause");
			playBtn.aspx(jQuery.mbYTPlayer.controls.play);
		...
		muteYTPVolume: function () {
			...
			muteBtn.aspx(jQuery.mbYTPlayer.controls.unmute);
		...
		unmuteYTPVolume: function () {
			...
			var controls = jQuery("#controlBar_" + YTPlayer.id);
			var muteBtn = controls.find(".mb_YTVPMuteUnmute");
			muteBtn.aspx(jQuery.mbYTPlayer.controls.mute);
		...
		buildYTPControls: function () {
			...
			var movieUrl = jQuery("<span/>").aspx(jQuery.mbYTPlayer.controls.ytLogo).addClass("mb_YTVPUrl ytpicon").attr("title", "view on YouTube").on("click", function () {window.open(vURL, "viewOnYT")});
			var onlyVideo = jQuery("<span/>").aspx(jQuery.mbYTPlayer.controls.onlyYT).addClass("mb_OnlyYT ytpicon").on("click",function () {jQuery(YTPlayer).fullscreen(data.realfullscreen);});
		...
		checkForState:function(YTPlayer){
			...
			YTPlayer.getState = setInterval(function () {
				...
				if(prog.totalTime){
					controlBar.find(".mb_YTVPTime").aspx(jQuery.mbYTPlayer.formatTime(prog.currentTime) + " / " + jQuery.mbYTPlayer.formatTime(prog.totalTime));
				} else{
					clearInterval(YTPlayer.getState);
					controlBar.find(".mb_YTVPTime").aspx("-- : -- / -- : --");
				}
