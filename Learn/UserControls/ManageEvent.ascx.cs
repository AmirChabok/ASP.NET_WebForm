...
public partial class UserControls_ManageEvents : System.Web.UI.UserControl
{
	...
	private void Initialize()
	{
		...
		try
		{
			...
			ddlPostCategory.SelectedValue = oPostRow.PostCategoryId.ToString();
			ddlPostType.SelectedValue = oPostRow.PostTypeId.ToString();
	...
	protected void btnDeletePost_Click(object sender, EventArgs e)
	{
		if (Page.IsValid)
		{
			try
			{
				...
				int intAffectedRows = oPostTableAdapter.DeleteByUserAndPostID((int)Session["UserID"], Convert.ToInt32(txtUserID.Text));
	...
	{
		Response.Redirect("~/Administrator/ManageEvents.aspx");
		return;
	}
